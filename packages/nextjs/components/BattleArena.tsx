...
export const BattleArena: React.FC<BattleArenaProps> = ({
  ...
  const projectiles = useMemo(() => {
    ...
    if (p1Moves[replayStep] >= 3) {
      activeProjectiles.push({
        id: `p1_${replayStep}`,
        owner: "p1",
        type: p1Moves[replayStep],
        lane: getPlayerHeight(p1Moves, replayStep),
      });
    }
    if (p2Moves[replayStep] >= 3) {
      activeProjectiles.push({
        id: `p2_${replayStep}`,
        owner: "p2",
        type: p2Moves[replayStep],
        lane: getPlayerHeight(p2Moves, replayStep),
      });
    }

    return activeProjectiles;
  }, [replayStep, p1Moves, p2Moves, isReplaying]);
  ...
  return (
    <div className="w-full h-96 bg-neutral-900 rounded-b-3xl border-2 border-t-0 border-primary/30 relative overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.6)] flex">
      ...
      >
        <div className="text-sm font-bold text-blue-300 mb-2 drop-shadow-lg tracking-widest w-max bg-black/60 px-3 py-1 rounded-full">
          {leftName}
        </div>
      ...
      >
        <div className="text-sm font-bold text-red-300 mb-2 drop-shadow-lg tracking-widest w-max bg-black/60 px-3 py-1 rounded-full">
          {rightName}
        </div>
